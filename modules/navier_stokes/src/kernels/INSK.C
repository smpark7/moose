...
INSK::computeQpResidual()
{
  ...
  Real convective_part = _rho[_qp] *
                         (_u_vel[_qp] * computeConcentrationGradient(_u, _grad_u, _qp)(0) +
                          _v_vel[_qp] * computeConcentrationGradient(_u, _grad_u, _qp)(1) +
                          _w_vel[_qp] * computeConcentrationGradient(_u, _grad_u, _qp)(2)) *
                         _test[_i][_qp];

  // The diffusive part
  Real eddy_visc = _rho[_qp] * _Cmu * computeConcentration(_u, _qp) *
                   computeConcentration(_u, _qp) / _epsilon[_qp];
  Real diffusive_part = _grad_test[_i][_qp] * (_mu[_qp] + eddy_visc / _sigk) *
                        computeConcentrationGradient(_u, _grad_u, _qp);

  // Turbulent dissipative sink part
  Real dissipative_part = _test[_i][_qp] * _rho[_qp] * _epsilon[_qp];
  // Real dissipative_part = 0.;
  ...
  Real source_part = -_test[_i][_qp] * 0.5 * eddy_visc * strain_tensor_double_dot_product;
...
INSK::computeQpJacobian()
{
  ...
  Real eddy_visc = _rho[_qp] * _Cmu * computeConcentration(_u, _qp) *
                   computeConcentration(_u, _qp) / _epsilon[_qp];
  Real d_eddy_visc_d_k = _rho[_qp] * _Cmu * 2. * computeConcentration(_u, _qp) / _epsilon[_qp] *
                         computeConcentrationDerivative(_u, _phi, _j, _qp);
  Real diffusive_part =
      _grad_test[_i][_qp] *
      ((_mu[_qp] + eddy_visc / _sigk) *
           computeConcentrationGradientDerivative(_u, _grad_u, _phi, _grad_phi, _j, _qp) +
       computeConcentrationGradient(_u, _grad_u, _qp) * d_eddy_visc_d_k / _sigk);
  ...
  Real source_part = -_test[_i][_qp] * 0.5 * d_eddy_visc_d_k * strain_tensor_double_dot_product;
...
INSK::computeQpOffDiagJacobian(unsigned jvar)
{
  ...
  if (jvar == _u_vel_var_number)
  {
    ...
    // Convective part
    Real convective_part =
        _phi[_j][_qp] * computeConcentrationGradient(_u, _grad_u, _qp)(p) * _test[_i][_qp];
    ...
    Real eddy_visc = _rho[_qp] * _Cmu * computeConcentration(_u, _qp) *
                     computeConcentration(_u, _qp) / _epsilon[_qp];
    Real source_part =
        -_test[_i][_qp] * 0.5 * eddy_visc * d_strain_tensor_double_dot_product_d_u_vel;
  ...
  else if (jvar == _v_vel_var_number)
  {
    ...
    Real eddy_visc = _rho[_qp] * _Cmu * computeConcentration(_u, _qp) *
                     computeConcentration(_u, _qp) / _epsilon[_qp];
    Real source_part =
        -_test[_i][_qp] * 0.5 * eddy_visc * d_strain_tensor_double_dot_product_d_v_vel;
  ...
  else if (jvar == _w_vel_var_number)
  {
    ...
    Real eddy_visc = _rho[_qp] * _Cmu * computeConcentration(_u, _qp) *
                     computeConcentration(_u, _qp) / _epsilon[_qp];
    Real source_part =
        -_test[_i][_qp] * 0.5 * eddy_visc * d_strain_tensor_double_dot_product_d_w_vel;
  ...
  {
    // The diffusive part
    Real d_eddy_visc_d_epsilon = -_rho[_qp] * _Cmu * computeConcentration(_u, _qp) *
                                 computeConcentration(_u, _qp) / std::pow(_epsilon[_qp], 2) *
                                 _phi[_j][_qp];
    Real diffusive_part = _grad_test[_i][_qp] * d_eddy_visc_d_epsilon / _sigk *
                          computeConcentrationGradient(_u, _grad_u, _qp);

    // Turbulent dissipative sink part
    Real dissipative_part = _test[_i][_qp] * _rho[_qp] * _phi[_j][_qp];
    // Real dissipative_part = 0.;
    ...
    Real source_part =
        -_test[_i][_qp] * 0.5 * d_eddy_visc_d_epsilon * strain_tensor_double_dot_product;

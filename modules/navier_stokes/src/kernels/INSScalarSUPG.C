...
INSScalarSUPG::INSScalarSUPG(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
INSScalarSUPG::computeQpResidual()
{
  ...
  Real convective_part = _rho[_qp] * U * computeConcentrationGradient(_u, _grad_u, _qp) * PG_test;

  return convective_part;
}

Real
INSScalarSUPG::computeQpJacobian()
{
  RealVectorValue U(_u_vel[_qp], _v_vel[_qp], _w_vel[_qp]);
  Real Pe = _rho[_qp] * _current_elem->hmax() * U.norm() / (2. * _mu[_qp]);
  Real delta = 1. / std::tanh(Pe) - 1. / Pe;
...
Real
INSScalarSUPG::computeQpOffDiagJacobian(unsigned jvar)
{
